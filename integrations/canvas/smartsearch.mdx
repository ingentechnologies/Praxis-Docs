...
1.  [Setup a Developer Key](#create-developer-keys) for API Smart Search.
2. [Configure each of your Praxis instances](#praxis-instance-configuration) with the `Key Client ID` and `Secret` to allow your digital twins to use the API Key for Smart Search.

   <Note>
     The developer key you will create is different from the LTI Key if you have installed Pria as an LTI
...
![Sign into Canvas](/images/integrations/canvas/smartsearch/canvas-ss-signin.webp)
  ...
  <Step title="Add Developer Key">
    **a.** In your account select `Developer Keys`.
        ![Select Developer Keys](/images/integrations/canvas/smartsearch/canvas-ss-select-dev-keys.webp)

    **b.** Click on the `+ Developer Key` button on the top right corner of the page.

        ![Add Developer Keys](/images/integrations/canvas/smartsearch/canvas-ss-add-dev-key.webp)
...
<Step title="Update API Key Settings">
  ...
  - **Redirect URI:** https://pria.praxislxp.com/api/auth/token_complete
...
Read more about [Canvas Scopes](#canvas-scopes) to understand the available scopes and their implications.
