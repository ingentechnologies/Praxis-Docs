...
1- Setup a Developer Key for API Smart Search. This key is different from the LTI Key if you have installed Pria as an LTI Activity for students.
...
## Developer Key for Smart Search APIs
...
1. Sign in to Canvas, ex: https://praxis-ai.instructure.com/
2. Once in the Canvas interface, click on `Admin` on the left tab, then select your account ex: 'Praxis AI'.
3. In your account select `Developer Keys`.
4. Click on the `+ Developer Key` button on the top right corner of the page.
5. Select + API key to create a new API Key. The new Key Settings wizard opens.
   Under `Key Name`, use `Pria Smart Search`. This name is visible in the oAuth Consent form _
   Under `Owner email` enter the admin email of your institution
   Under `Redirect URL` enter this URL: https://pria.praxislxp.com/api/auth/token_complete
   Under `Icon URL` enter the Pria icon : https://pria.praxislxp.com/logo192.png This Icon is displayed to learners in the oAuth consent form _
   Leave the rest empty like below.
   \*: This is the consent form presented to students upon authorization

6. Checkbox `Enforced Scope` located to the right of Key Name.
7. After you check `Enforce Scope`, Type `smart` in the search box to locate the Smart Search API function
8. Check the `Smart Search` box to grant access to the Smart Search function to your key.
9. Click `Save` on bottom right corner of the page.
10. Change the state of your developer key to enabled to enable this API Key
...
Think of them as granular permission settings that follow the principle of least privilege – applications should only have access to what they absolutely need.
...
5. Click `Update` at the bottom, and Voila!
