...
<Note>This key is different from the LTI Key if you have installed Pria as an LTI Activity for students.</Note>
...
<Steps>
  ...
  <Step title="Add Developer Key">
    - In your account select `Developer Keys`.
    - Click on the `+ Developer Key` button on the top right corner of the page.
    - Select `+ API` key to create a new API Key. 
  </Step>

<Step title="Update Key Settings">
...
<Step title="Enforce Scopes">
    - Checkbox `Enforced Scope` located to the right of Key Name.
    - After you check `Enforce Scope`, Type `smart` in the search box to locate the Smart Search API function
    - Check the `Smart Search` box to grant access to the Smart Search function to your key.
    - Click `Save` on bottom right corner of the page.
    - Change the state of your developer key to enabled to enable this API Key
