...
To add a Instance globally to a Canvas account, inject a segment of Javascript code in the Theme used by the account or sub-account of your choice.
...
<Steps>
<Step title="Login to Canvas as an Administrator">
...
Download the pria-sdk-canvas.js code at https://pria.praxislxp.com/pria-sdk-canvas.js onto your computer, and save the file as pria-sdk-canvas.js 
</Step>
</Steps>

<Note>
- You may save this file into a Google Drive, and edit your customizations on an ongoing basis. 
...
<Note>
- We recommend using a square image, no larger than 192 x 192 pixels, that also supports transparency (like PNG or webp format)
- This image should be placed on a Content Delivery Network that allows access across origin or domains (CORS Policy)
</Note>
...
<Note>the loadPria() function will not load Pria twice if it happens to be called more than once.</Note>
...
<Note>In Canvas, course pages have URLS that match this structure https://my-univertivt.canvas.edu/courses/106/… and this is what we rely on to determine to loadPria() so it loads only for courses.</Note>
